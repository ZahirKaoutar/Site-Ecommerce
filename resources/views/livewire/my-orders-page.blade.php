                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  @foreach ( $orders as $order)
                        ...
                        $payement_status='';
                            ...
                            if($order->status == 'processing'){
                             ...
                             if($order->status == 'shipped'){
                                    $status ='<span class="bg-green-700 py-1 px-3 rounded text-white shadow">Shippedd</span>'; }


                            if($order->status == 'delivered'){
                                        $status ='<span class="bg-green-500 py-1 px-3 rounded text-white shadow">Delivred</span>'; }

                            if($order->status == 'canceled'){
                                        $status ='<span class="bg-red-500 py-1 px-3 rounded text-white shadow">Canceled</span>'; }
                            if($order->payement_status == 'pending'){
                                $payement_status='<span class="bg-blue-500 py-1 px-3 rounded text-white shadow">Pending</span>'; }
                            if($order->payement_status == 'paid'){
                                    $payement_status='<span class="bg-green-500 py-1 px-3 rounded text-white shadow">Paid</span>'; }

                            if($order->payement_status == 'failled'){
                                        $payement_status='<span class="bg-red-500 py-1 px-3 rounded text-white shadow">Pending</span>'; }
                    @endphp
                    <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-900 dark:even:bg-slate-800" wire:key='{{$order->id}}'>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{!! $payement_status !!}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{{Number::currency($order->grand_totale,"MAD")}}</td>
